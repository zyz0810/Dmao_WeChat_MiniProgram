  ...
  onLoad: function(options) {
    ...
    if (codetype == 'password') {
      ...
      new member(function(data) {
        ...
        that.setData({
          receivePhone: wx.getStorageSync('phone')
        })
        countdown(that);
        wx.showToast({
          title: '发送成功',
    ...
    } else if (codetype == 'loginPassword') {
      wx.setNavigationBarTitle({
        title: '修改登录密码'
      });
    ...
    if (e.detail.value.length == 6) {
      ...
      if (that.data.codetype == 'password') {
        new member(function(data) {
          ...
          wx.redirectTo({
            url: '/pages/member/password/password?captcha=' + that.data.code + '&codeType=password',
          })
        }, function(data) {
        ...
        new login(function(data) {
          ...
          wx.redirectTo({
            url: '/pages/member/password/password?captcha=' + that.data.code + '&codeType=loginPassword',
          })
        }, function(data) {
          if (data.message.type == 'error') {
            that.setData({
